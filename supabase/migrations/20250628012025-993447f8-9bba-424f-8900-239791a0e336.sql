...
-- Create enhanced function to get all brand applications regardless of status
CREATE OR REPLACE FUNCTION get_brand_applications_all(
...
BEGIN
  ...
  WHERE bu.user_id = auth.uid()
    AND cp.status IN ('applied', 'invited', 'pending', 'approved', 'rejected', 'completed', 'active', 'accepted')
...
GRANT EXECUTE ON FUNCTION get_brand_applications_all TO authenticated;

-- Fix the get_campaign_waiting_influencers function type mismatch
CREATE OR REPLACE FUNCTION public.get_campaign_waiting_influencers(campaign_id_param uuid)
  ...
  SUBSTRING(cp.influencer_id::text, 1, 8)) as influencer_handle,
   ...
   % 400) / 100.0)::numeric, 1)
    ...
    WHERE cp.campaign_id = campaign_id_param
      ...
      AND cp.current_stage = 'waiting_for_requirements'

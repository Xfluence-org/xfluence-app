-- Fix the get_brand_applications function to return niche as an array
...
RETURNS TABLE (
    ...
    niche TEXT[],  -- Changed to TEXT[] array
    ai_score NUMERIC
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    SELECT 
        ...
        ARRAY[COALESCE(p.name, 'General')] as niche,  -- Return as array
        COALESCE(cp.ai_match_score::numeric, 0::numeric) as ai_score
